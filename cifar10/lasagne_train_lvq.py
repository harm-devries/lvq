...
def main(num_epochs=300, lr_start=2e-1, lr_end=1e-6, momentum=0.9, optimizer='momentum'):
    ...
    lamb = theano.shared(numpy.float32(0.1))
    from sng import SNG, initialize_prototypes
    sng = SNG(10, 200, nonlin=True, gamma=2.25, lamb=lamb)
    _, train_loss, train_misclass = sng.propagate(train_out, flat_y)
    _, test_loss, test_misclass = sng.propagate(test_out, flat_y)
    initialize_prototypes(sng, x, train_out, iterate_minibatches(X_train, y_train, 100))
    ...
    if optimizer == 'adam':
        updates = lasagne.updates.adam(train_loss+reg, params, learning_rate=lr)
    else:
        updates = lasagne.updates.momentum(train_loss+reg, params, lr, momentum)
        
    train_fn = theano.function([x, y], [train_loss, train_misclass], updates=updates)
    ...
    for epoch in range(num_epochs):
        ...
        lamb.set_value(numpy.float32(lamb.get_value()+0.02))
        print lamb.get_value()
        ...
        if (best_val_err is None or val_err <= best_val_err):
            ...
            numpy.savez('/Tmp/devries/lvq_nonlin_raw2.npz', *lasagne.layers.get_all_param_values(network) + [sng.W.get_value()])
