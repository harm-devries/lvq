...
def load_data():
    import cPickle
    d = cPickle.load(open("/Tmp/devries/cifar10.pkl", 'rb')) 
...
def main(num_epochs=350, lr_start=3e-3, lr_end=2e-6, momentum=0.9):
    ...
    updates = lasagne.updates.adam(
        train_nll+reg, params, lr, beta1=momentum)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 100, shuffle=True):
            ...
            nll, err = train_fn(inputs, targets)
            train_nll += nll
            train_err += err
            train_batches += 1
        ...
        print("  validation error:\t\t{:.2f} %".format(val_err / val_batches))
