...
layers = [ConvolutionalActivation(act, (3, 3), 64, name='l1'), 
          ConvolutionalActivation(act, (3, 3), 64, name='l2'),
          MaxPooling((2, 2)),
          ConvolutionalActivation(act, (3, 3), 128, name='l3'),
          ConvolutionalActivation(act, (3, 3), 128, name='l4'),
          MaxPooling((2, 2)),
...
flat = Flattener()
train_out = flat.apply(conv_sequence.training(x))
test_out = flat.apply(conv_sequence.inference(x))

mlp = MLP(activations=[None], dims=[256, 100])
...
step_rule = Momentum(1e-2, 0.9) #Adam(learning_rate=lr) #RMSProp(learning_rate=1e-5, max_scaling=1e4)
...
main_loop = MainLoop(
     ...
     extensions=[FinishAfter(after_n_epochs=150),
                 ...
                 LRDecay(step_rule.learning_rate, 100, 150), 
                 ...
                 DataStreamMonitoring(
                    ...
                    prefix='test'),
                 Timing(), 
                 EarlyStopping('test_misclass', 100, './exp/softmax_2_bn_cifar100.pkl'),
